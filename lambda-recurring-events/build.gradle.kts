...
application {
    mainClass = "com.shiftorganization.lambda.recurringevents.MainKt"
}

tasks.shadowJar {
    archiveBaseName = "recurring-events"
    mergeServiceFiles()
    manifest {
        attributes["Main-Class"] = "com.shiftorganization.lambda.recurringevents.RecurringEventsModuleKt"
    }
}
...
if (project.hasProperty("native")) {
    extensions.configure<org.graalvm.buildtools.gradle.dsl.GraalVMExtension> {
        binaries {
            named("main") {
                imageName.set("recurring-events")
